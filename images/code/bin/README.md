...
扩展名，如 `skills.py` → `/usr/local/bin/skills`）。
...
- 本体由上游仓维护并随版本同步到这里（上游统一在 createrole 仓（dev）
  ...
  - `skills` — `server/infrastructure/sandbox_cli/skills.py`（见 issue #15/#18/#24；原 `skillhub`，
    #24 改名且**不留别名**，为第三方 skillhub.cn 的同名 CLI 让出命名空间）；
